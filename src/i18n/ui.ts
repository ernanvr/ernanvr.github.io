import { DEFAULT_LOCALE, type Locale } from "./utils";

const ui = {
  en: {
    "nav.skipToContent": "Skip to content",
    "nav.openMenu": "Open Menu",
    "nav.closeMenu": "Close Menu",
    "nav.posts": "Posts",
    "nav.tags": "Tags",
    "nav.about": "About",
    "nav.uses": "Uses",
    "nav.archives": "Archives",
    "nav.search": "Search",
    "nav.language": "Language",
    "nav.themeToggle": "Toggle light and dark mode",
    "page.tags.title": "Tags",
    "page.tags.desc": "All the tags used in posts.",
    "page.archives.title": "Archives",
    "page.archives.desc": "All the articles I've archived.",
    "page.archives.empty": "No archived articles yet.",
    "page.search.title": "Search",
    "page.search.desc": "Search any article ...",
    "page.home.title": "Home",
    "home.socialLinks": "Social Links:",
    "home.featured": "Featured",
    "home.recentPosts": "Recent Posts",
    "home.allPosts": "All Posts",
    "post.goBack": "Go back",
    "post.backToTop": "Back to Top",
    "post.previousPost": "Previous Post",
    "post.nextPost": "Next Post",
    "post.share": "Share this post on:",
    "pagination.prev": "Prev",
    "pagination.next": "Next",
    "search.search": "Search",
    "search.placeholder": "Search for anything...",
    "search.found": "Found",
    "search.result": "result",
    "search.results": "results",
    "search.for": "for",
    "datetime.updated": "Updated:",
    "datetime.published": "Published:",
    "footer.rights": "All rights reserved.",
    "breadcrumbs.home": "Home",
    "breadcrumbs.page": "page",
  },
  es: {
    "nav.skipToContent": "Saltar al contenido",
    "nav.openMenu": "Abrir menu",
    "nav.closeMenu": "Cerrar menu",
    "nav.posts": "Articulos",
    "nav.tags": "Etiquetas",
    "nav.about": "Sobre mí",
    "nav.uses": "Uso",
    "nav.archives": "Archivos",
    "nav.search": "Buscar",
    "nav.language": "Idioma",
    "nav.themeToggle": "Cambiar modo claro y oscuro",
    "page.tags.title": "Etiquetas",
    "page.tags.desc": "Todas las etiquetas usadas en articulos.",
    "page.archives.title": "Archivos",
    "page.archives.desc": "Todos los articulos archivados.",
    "page.archives.empty": "Aún no hay artículos archivados.",
    "page.search.title": "Buscar",
    "page.search.desc": "Busca cualquier articulo ...",
    "page.home.title": "Inicio",
    "home.socialLinks": "Redes:",
    "home.featured": "Destacado",
    "home.recentPosts": "Articulos recientes",
    "home.allPosts": "Todos los articulos",
    "post.goBack": "Regresar",
    "post.backToTop": "Volver arriba",
    "post.previousPost": "Articulo anterior",
    "post.nextPost": "Siguiente articulo",
    "post.share": "Compartir este articulo en:",
    "pagination.prev": "Anterior",
    "pagination.next": "Siguiente",
    "search.search": "Buscar",
    "search.placeholder": "Busca cualquier tema...",
    "search.found": "Encontrados",
    "search.result": "resultado",
    "search.results": "resultados",
    "search.for": "para",
    "datetime.updated": "Actualizado:",
    "datetime.published": "Publicado:",
    "footer.rights": "Todos los derechos reservados.",
    "breadcrumbs.home": "Inicio",
    "breadcrumbs.page": "pagina",
  },
} as const;

type TranslationKey = keyof (typeof ui)[typeof DEFAULT_LOCALE];

const translators = new Map<Locale, (key: TranslationKey) => string>();

export const useTranslations = (locale: Locale) => {
  const cached = translators.get(locale);
  if (cached) return cached;

  const translator = (key: TranslationKey) =>
    ui[locale][key] ?? ui[DEFAULT_LOCALE][key];
  translators.set(locale, translator);
  return translator;
};
